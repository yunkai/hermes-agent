...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _listen_loop(self) -> None:
        ...
        Close code handling follows the OpenClaw qqbot reference implementation:
          ...
          4009 → session timed out, preserve session for Resume (not invalid)
        ...
        while self._running:
            try:
                ...
                # Session timed out → preserve session, will Resume on reconnect
                # 4009 is a timeout (not invalidation) — see OpenClaw PR #65606.
                # Resuming the session avoids a full re-identify round-trip.
                if code == 4009:
                    logger.info(
                        "[%s] Session timed out (4009), preserving session for Resume",
                        self._log_tag,
                    )
